...
├── sw.js            # Service Worker (cache-first, version: habit-tracker-v2.0)
...
- **Service Worker** (`sw.js`): Cache-first. Cache name: `habit-tracker-v2.0` — bump to force refresh
...
| `isItemActiveOnDate(item, dateStr)` | Check if item is active on a given effective date via `activeDays` |
