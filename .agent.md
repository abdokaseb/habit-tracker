...
- **`settings.resetHour`**: 0–23, when the "day" rolls over (default 0 = midnight)
  - **How it works**: `getEffectiveDate()` returns yesterday if `now.getHours() < resetHour`; otherwise today.
  - **Storage key**: completions are stored under the calendar date returned by `getEffectiveDate()` at the moment of completion.
  - **Day-of-week mapping** (critical for `resetHour > 13`): `isItemActiveOnDate()` and `effectiveDayOfWeek()` add **+1 calendar day** before checking `activeDays`. This maps stored dates back to their logical weekday.
  - **Example with resetHour=18 (6 PM)**:
    | Wall clock | Calendar date | Effective date stored | Logical weekday checked |
    |---|---|---|---|
    | Fri 7 PM | Friday | Friday | Saturday (Fri+1) |
    | Sat 3 PM | Saturday | Friday (before 6 PM) | Saturday (Fri+1) |
    | Sat 7 PM | Saturday | Saturday | Sunday (Sat+1) |
  - So a "Saturday habit" (`activeDays=[6]`) stores completions as the **Friday calendar date**. The +1 offset in `effectiveDayOfWeek()` converts it back correctly.
  - **Rule**: Never use raw `getDay()` when checking `activeDays` against stored dates. Always use `effectiveDayOfWeek(dateStr, settings)` or `isItemActiveOnDate()`.
...
| `calcStreak(dates, settings, activeDays)` | Consecutive-day streak for a habit |
...
| `isItemActiveOnDate(item, dateStr, settings)` | Check if item is active on a given effective date via `activeDays` (applies resetHour > 13 offset) |
| `effectiveDayOfWeek(dateStr, settings)` | Returns the logical weekday for a stored date string (applies resetHour > 13 +1 offset) — use instead of raw `getDay()` when matching against `activeDays` |
| `calendarDateToEffectiveDate(calDateStr, settings)` | Converts a physical calendar cell date to the stored effective date (inverse of +1 shift). Use in `toggleCalendarDay` and heatmap display. |
